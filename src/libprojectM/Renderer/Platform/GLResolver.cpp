...
auto IsLikelyExtensionName(const char* name) -> bool
{
    ...
    static constexpr std::array<std::string_view, 3> kPrefixes = {
        "GL_", "WEBGL_", "ANGLE_"};
    ...
    {
        if (nameSv.substr(0, prefix.size()) == prefix)
        {
            return true;
        }
    }
    ...
    static constexpr std::array<std::string_view, 33> kSuffixes = {
    ...
    {
        if (nameSv.size() >= suffix.size() &&
            nameSv.substr(nameSv.size() - suffix.size()) == suffix)
        {
            return true;
        }
    }
